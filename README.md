...
5. Your browser opens automatically. Your laptop's own webcam works immediately for barcode scanning — no extra setup needed.
...
```
server.js              Express backend: book lookup, Claude analysis, library storage
...
data/library.json       Your saved books (created automatically, gitignored)
```
...
The **Family** tab lets you add each kid (name + age) and set family-wide thresholds: the minimum age at which you would be comfortable with a child encountering each content level. Every analysis then shows a verdict per kid — "Good to go" or "Worth discussing first" — instead of just raw category levels. Choose **"Always flag"** on any level to flag it regardless of a kid's age — useful for anything your family considers off-limits outright, not just age-gated.
